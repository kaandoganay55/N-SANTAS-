...
const categories = [
  ...
  { key: 'babet', label: 'Babet' },
  { key: 'oxford', label: 'Oxford' },
  { key: 'topuklu', label: 'Topuklu' },
  { key: 'terlik', label: 'Terlik' },
  { key: 'sandalet', label: 'Sandalet' },
  { key: 'spor', label: 'Spor' }
];
...
export default function TumUrunlerPage() {
  ...
  }, [products, searchQuery, selectedCategory, selectedPriceRange, selectedSizes, showOnlyInStock, showOnlyOnSale, showOnlyNew, sortBy]);
  ...
  const hasActiveFilters = searchQuery || selectedCategory !== 'all' || selectedPriceRange !== 'all' || 
    selectedSizes.length > 0 || showOnlyInStock || showOnlyOnSale || showOnlyNew;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-white to-stone-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-stone-900 mb-4">Tüm Ürünler</h1>
          <p className="text-xl text-stone-600">{filteredProducts.length} ürün bulundu</p>
        </div>

        {/* Search Bar */}
        <div className="bg-white rounded-3xl shadow-lg border border-stone-100 p-6 mb-8">
          <div className="relative">
            <FiSearch className="absolute left-4 top-1/2 transform -translate-y-1/2 h-5 w-5 text-stone-400" />
            <input
              type="text"
              placeholder="Ürün ara..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-12 pr-4 py-4 border border-stone-300 rounded-2xl focus:outline-none focus:ring-2 focus:ring-stone-500 focus:border-transparent text-stone-900 placeholder-stone-400"
            />
          </div>
        </div>
        ...
        <div className="lg:grid lg:grid-cols-4 lg:gap-8">
          ...
          <div className={`lg:col-span-1 ${showMobileFilters ? 'block' : 'hidden lg:block'}`}>
            <div className="bg-white rounded-3xl shadow-lg border border-stone-100 p-6">
              ...
              {/* Categories */}
              <div className="mb-8">
                <h3 className="text-lg font-semibold text-stone-900 mb-4">Kategoriler</h3>
                    ...
                    <div className="flex items-center space-x-6">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 text-sm text-stone-600">
                          <span>Kategori: {categories.find(c => c.key === product.category)?.label}</span>
                          {product.rating && (
